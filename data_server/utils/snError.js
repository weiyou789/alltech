...
module.exports = function (name, err) {
    ...
    if ((err && constant[err.message] && constant[err.message].code) || (err && constant[err.message] && constant[err.message].desc)) {
        //constant捕捉到错误
        let cont = `${now}|${uuidV1()}|${now}|${pkg.sysCode}|${code}|2|${'snError_debug'}|||||${name}|${message}|${(err && err.code) || err.message}|${err && err.stack}|||||2||api|0.0.0.4`;
        process.nextTick(() => {
            logger.debug(cont)
        });
    } else {
        let cont = `${now}|${uuidV1()}|${now}|${pkg.sysCode}|${(err && err.code) || code}|1|${toBase64((err && err.name) || 'snError')}|||||${name}|${message}|${(err && err.code) || (err && err.message)}|${toBase64(err && err.stack)}|||||1||api|0.0.0.4`;
        process.nextTick(() => {
            logger.error(cont)
        });
    }
